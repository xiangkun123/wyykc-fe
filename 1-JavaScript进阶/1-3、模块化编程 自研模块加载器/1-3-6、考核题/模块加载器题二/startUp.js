(function(global) {
	...
	function getDataMain() {
		...
		for (; i < scripts.length; i++) {
			if(scripts[i].getAttribute('data-main')) {				// 是否配置data-main
				// 最后一个字符是否是 "/"
				str = scripts[i].getAttribute('data-main');
				strLen = str.length-1;
				dataMain = str.charAt(strLen) === "/" ? str : str + "/";
			}
		}
		return dataMain;
	}
	...
	startUp.resolve = function(child, parent) {
		...
		if (!child) return "";
		// 是否配置了data-main，有就直接用data-main，否则就经过别名和路径短名称别名
		if(dataMain) {
			child = dataMain + child;
		} else {
