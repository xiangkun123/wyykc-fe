(function(root) {
  ...
  var templateSettings = {
    interpolate: /<%=([\s\S]+?)%>/g,      // 变量捕获
    escape: /<%-([\s\S]+?)%>/g,           // 逃逸字符捕获
    evalute: /<%([\s\S]+?)%>/g,           // js代码捕获
  }
  ...
  _.template = function(templateString, settings) {
    ...
    // 合并用户配置到默认配置中
    var settings = _.extend({}, templateSettings, settings);

    // 将模式存在数组，变成一个字符串，再通过RegExp变成正则给到match变量
    // evalute匹配范围最广，需要放到最后匹配
  ...
  _.each(["Function", "String", "Number", "Boolean", "Arguments"], function(name) {
